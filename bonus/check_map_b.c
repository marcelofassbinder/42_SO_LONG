...
/*   Updated: 2024/03/26 13:24:16 by marcelo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map(t_map map)
{
	...
	if (map.line < 3 || map.column < 3)
		return(error_message("Map must contain more than 3 lines/columns.\n", map));
		...
		while(map_array[i][j])
		{
			if(map_array[i][j] != WALL && map_array[i][j] != EMPTY && 
				map_array[i][j] != PLAYER && map_array[i][j] != EXIT &&
					map_array[i][j] != COLLECTIBLE && map_array[i][j] != JELLYFISH)
				return(0);
			j++;
		}
		i++;
